    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-neutral-900 z-50 shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-neutral-700">
          <h2 className="text-xl font-bold text-white">{cartTitle}</h2>
          <button
            type="button"
            onClick={() => setIsOpen(false)}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-white/60">{emptyCart}</p>
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="mt-4 text-fuchsia-400 hover:text-fuchsia-300 transition-colors"
              >
                {continueShopping}
              </button>
            </div>
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="bg-neutral-800 rounded-xl p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      {item.originalPrice && (
                        <p className="text-sm text-white/40 line-through">{(item.originalPrice * item.quantity).toFixed(2)}</p>
                      )}
        ...
        {items.length > 0 && (
          <div className="border-t border-neutral-700 p-6 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-white/70">{totalText}</span>
              <span className="text-2xl font-bold text-white">{totalPrice.toFixed(2)}</span>
            </div>
            <Link
              href="/checkout"
              onClick={() => setIsOpen(false)}
              className="block w-full bg-gradient-to-r from-pink-500 via-fuchsia-500 to-violet-500 hover:from-pink-400 hover:via-fuchsia-400 hover:to-violet-400 text-white text-center py-4 rounded-full font-semibold transition-all"
            >
              {checkoutText}
            </Link>
            <button
              type="button"
              onClick={() => setIsOpen(false)}
              className="block w-full text-center text-white/60 hover:text-white transition-colors py-2"
            >
              {continueShopping}
            </button>
          </div>
